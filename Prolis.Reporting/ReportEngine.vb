...
#Region "Public Report Generation Method"
    ...
    Public Async Function GenerateReportsAsync(
        ...
        isCustomRPT As Boolean, ReportPath As String
        ...
        For Each accID As String In accIDs
            ...
            Try
                ...
                Dim rptFile As String = Path.Combine(ReportPath, "Reports", ValidateReportFile(accID, isCustomRPT))
                If Not File.Exists(rptFile) Then
                    Throw New FileNotFoundException($"The required report file was not found.", rptFile)
                End If
                Dim d = New ReportDocument()

                Using oRpt As New ReportDocument()
                    oRpt.Load(rptFile, OpenReportMethod.OpenReportByDefault)

                    ApplyNewServer(oRpt, _connInfo.DSN, _connInfo.DbName, _connInfo.DbUser, _connInfo.PWD)
                    oRpt.SummaryInfo.ReportTitle = Await GetFlagInfoAsync(accID)
                    oRpt.RecordSelectionFormula = "{Requisitions.ID} = " & accID
                    'oRpt.ExportToDisk(ExportFormatType.PortableDocFormat, accID & ".pdf")
