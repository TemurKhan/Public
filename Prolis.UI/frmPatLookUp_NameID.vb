...
Public Class frmPatLookUp_NameID
    ...
    Friend Sub FindPatientByName(ByVal PatName As String)
        ...
        End If
        dgv.Rows.Clear()
        Dim cnfp As New SqlClient.SqlConnection(connString)
        cnfp.Open()
        Dim cmdfp As New SqlClient.SqlCommand(sSQL, cnfp)
        cmdfp.CommandType = CommandType.Text
        Dim drfp As SqlClient.SqlDataReader = cmdfp.ExecuteReader
        If drfp.HasRows Then
            While drfp.Read
                'If drfp("Address_ID") Is DBNull.Value Then
                '    dgvPatients.Rows.Add(drfp("ID"), drfp("LastName"), drfp("FirstName"), _
                '    Format(drfp("DOB"), SystemConfig.DateFormat), GetFullGender(drfp("Sex")), _
                '    drfp("SSN"), "")
                'Else
                '<<<<<<< HEAD
                dgv.Rows.Add(drfp("ID"), drfp("LastName"), drfp("FirstName"),
                Format(drfp("DOB"), SystemConfig.DateFormat), GetFullGender(drfp("Sex")),
                drfp("SSN"), drfp("Address"))      'GetAddress(drfp("Address_ID"))
                '=======
                Dim ad = drfp("ID")
    ...
    Private Sub FindPatientBySSN(ByVal SSN As String)
        dgv.Rows.Clear()
        If Trim(SSN) <> "" Then
            Dim cnssn As New SqlClient.SqlConnection(connString)
            cnssn.Open()
            Dim cmdssn As New SqlClient.SqlCommand("Select " &
            "* from vw_Patients where SSN = '" & SSN & "'", cnssn)
            cmdssn.CommandType = CommandType.Text
            Dim drssn As SqlClient.SqlDataReader = cmdssn.ExecuteReader
    ...
    Private Sub dgvPatients_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgv.CellClick
        If e.RowIndex <> -1 Then
            Me.Tag = dgv.Rows(e.RowIndex).Cells(0).Value.ToString + "|" + dgv.Rows(e.RowIndex).Cells(1).Value.ToString + "|" + dgv.Rows(e.RowIndex).Cells(2).Value.ToString + "|" + dgv.Rows(e.RowIndex).Cells(3).Value.ToString
